...
namespace std
{
    template class _CRTIMP2
        basic_string<unsigned short, char_traits<unsigned short>, allocator<unsigned short> >;

    template class _CRTIMP2 basic_string<__wchar_t, char_traits<__wchar_t>, allocator<__wchar_t> >;
}  // namespace std
...
// for any compiler that provides this, wchar_t is guaranteed to hold any Unicode value with a single
// code point (32-bit or larger) so we can safely skip the rest of the testing
#    else  // #ifdef __STDC_ISO_10646__
#        if defined( __WIN32__ ) || defined( _WIN32 ) || !defined( ANDROID ) || !defined( __OHOS__ )
#            define WCHAR_UTF16  // All currently known Windows platforms utilize UTF-16 encoding in
                                 // wchar_t
#        else                    // #if defined( __WIN32__ ) || defined( _WIN32 )
#            if OGRE_COMPILER != OGRE_COMPILER_GCCE
#                if WCHAR_MAX <= \
                    0xFFFF  // this is a last resort fall back test; WCHAR_MAX is defined in <wchar.h>
...
#    endif  // OGRE_COMPILER == OGRE_COMPILER_MSVC
    ...
    */
    ...
    public:
        ...
        class _OgreExport invalid_data : public std::runtime_error
        ...
        class _OgreExport _fwd_iterator : public _base_iterator
        { /* i don't know why the beautifier is freaking out on this line */
        ...
        public:
            ...
            //! Returns the Unicode value of the character at the current position (decodes surrogate
            //! pairs if needed)
            unicode_char getCharacter() const;
            //! Sets the Unicode value of the character at the current position (adding a surrogate pair
            //! if needed); returns the amount of string length change caused by the operation
        ...
        public:
            ...
            //! difference operator
            friend size_type operator-( const _const_fwd_iterator &left,
        ...
        public:
            ...
            //! difference operator
            friend size_type operator-( const _const_rev_iterator &left,
...
#    endif
        ...
        //! returns the number of elements that the string can hold before it will need to allocate more
        //! space
        ...
        /*! If \a num is omitted, it will default to \c UTFString::npos, and the substr() function will
         * simply return the remainder of the string starting at \a index. */
...
#    endif
        ...
        of a lead code point that is followed by a trailing code point. Evaluating the trailing code
        point itself, or pointing to a code point that is a sentinel value (part of a broken pair) will
        return the value of just that code point (not a valid Unicode value, but useful as a sentinel
        value). */
        unicode_char getChar( size_type loc ) const;
        //! sets the value of the character at \a loc to the Unicode value \a ch (UTF-32)
        /*! Providing sentinel values (values between U+D800-U+DFFF) are accepted, but you should be
        aware that you can also unwittingly create a valid surrogate pair if you don't pay attention to
        what you are doing. @note This operation may also lengthen the string if a surrogate pair is
        needed to represent the value given, but one is not available to replace; or alternatively
        shorten the string if an existing surrogate pair is replaced with a character that is
        representable without a surrogate pair. The return value will signify any lengthening or
        shortening performed, returning 0 if no change was made, -1 if the string was shortened, or 1 if
        the string was lengthened. Any single call can only change the string length by + or - 1. */
...
#    endif
        ...
        //! assign the first \a num characters of \a c_str to the current string (\a c_str is treated as
        //! a UTF-8 stream)
        ...
        //! appends a substring of \a str starting at \a index that is \a len characters long on to the
...
#    endif
        ...
        //! appends \a num repetitions of \a ch on to the end of the current string (Unicode values less
        //! than 128)
        UTFString &append( size_type num, char ch );
        //! appends \a num repetitions of \a ch on to the end of the current string (Full Unicode
        //! spectrum)
        ...
        //! inserts a substring of \a str (starting at \a index2 and \a num code points long) into the
        //! current string, at location \a index1
        UTFString &insert( size_type index1, const UTFString &str, size_type index2, size_type num );
        //! inserts the code points denoted by \a start and \a end into the current string, before the
        //! code point specified by \a i
...
#    endif
        ...
        //! removes the code points between \a start and \a end (including the one at \a start but not
        //! the one at \a end), returning an iterator to the code point after the last code point removed
        ...
        //! replaces up to \a num1 code points of the current string (starting at \a index1) with up to
        //! \a num2 code points from \a str
        UTFString &replace( size_type index1, size_type num1, const UTFString &str, size_type num2 );
        //! replaces up to \a num1 code points of the current string (starting at \a index1) with up to
        //! \a num2 code points from \a str beginning at \a index2
        UTFString &replace( size_type index1, size_type num1, const UTFString &str, size_type index2,
                            size_type num2 );
        //! replaces code points in the current string from \a start to \a end with \a num code points
        //! from \a str
        UTFString &replace( iterator start, iterator end, const UTFString &str, size_type num = npos );
        //! replaces up to \a num1 code points in the current string (beginning at \a index) with \c num2
        //! copies of \c ch
        UTFString &replace( size_type index, size_type num1, size_type num2, code_point ch );
        //! replaces the code points in the current string from \a start to \a end with \a num copies of
        //! \a ch
        ...
        //! compare \a str to a substring of the current string, starting at \a index for \a length
        //! characters
        int compare( size_type index, size_type length, const UTFString &str ) const;
        //! compare a substring of \a str to a substring of the current string, where \a index2 and \a
        //! length2 refer to \a str and \a index and \a length refer to the current string
        int compare( size_type index, size_type length, const UTFString &str, size_type index2,
                     size_type length2 ) const;
        //! compare a substring of \a str to a substring of the current string, where the substring of \a
        //! str begins at zero and is \a length2 characters long, and the substring of the current string
        //! begins at \a index and is \a length  characters long
        int compare( size_type index, size_type length, const code_point *str, size_type length2 ) const;
#    if OGRE_IS_NATIVE_WCHAR_T
        //! compare a substring of \a str to a substring of the current string, where the substring of \a
        //! str begins at zero and is \a length2 elements long, and the substring of the current string
        //! begins at \a index and is \a length characters long
        int compare( size_type index, size_type length, const wchar_t *w_str, size_type length2 ) const;
#    endif
        //! compare a substring of \a str to a substring of the current string, where the substring of \a
        //! str begins at zero and is \a length2 <b>UTF-8 code points</b> long, and the substring of the
        //! current string begins at \a index and is \a length characters long
        ...
        //!\name find & rfind
        //@{
        //! returns the index of the first occurrence of \a str within the current string, starting at \a
        //! index; returns \c UTFString::npos if nothing is found
        /*! \a str is a UTF-16 encoded string, but through implicit casting can also be a UTF-8 encoded
         * string (const char* or std::string) */
        size_type find( const UTFString &str, size_type index = 0 ) const;
        //! returns the index of the first occurrence of \a str within the current string and within \a
        //! length code points, starting at \a index; returns \c UTFString::npos if nothing is found
...
#    endif
        //! returns the index of the first occurrence \a ch within the current string, starting at \a
        //! index; returns \c UTFString::npos if nothing is found
...
#    endif
        ...
        //! returns the location of the first occurrence of \a str in the current string, doing a reverse
        //! search from \a index; returns \c UTFString::npos if nothing is found
        size_type rfind( const UTFString &str, size_type index = 0 ) const;
        //! returns the location of the first occurrence of \a str in the current string, doing a reverse
        //! search from \a index, searching at most \a num characters; returns \c UTFString::npos if
        //! nothing is found
...
#    endif
        //! returns the location of the first occurrence of \a ch in the current string, doing a reverse
        //! search from \a index; returns \c UTFString::npos if nothing is found
...
#    endif
        ...
        //@{
        //! Returns the index of the first character within the current string that matches \b any
        //! character in \a str, beginning the search at \a index and searching at most \a num
        //! characters; returns \c UTFString::npos if nothing is found
        size_type find_first_of( const UTFString &str, size_type index = 0, size_type num = npos ) const;
        //! returns the index of the first occurrence of \a ch in the current string, starting the search
        //! at \a index; returns \c UTFString::npos if nothing is found
...
#    endif
        ...
        //! returns the index of the first character within the current string that does not match any
        //! character in \a str, beginning the search at \a index and searching at most \a num
        //! characters; returns \c UTFString::npos if nothing is found
        size_type find_first_not_of( const UTFString &str, size_type index = 0,
                                     size_type num = npos ) const;
        //! returns the index of the first character within the current string that does not match \a ch,
        //! starting the search at \a index; returns \c UTFString::npos if nothing is found
...
#    endif
        ...
        //! returns the index of the first character within the current string that matches any character
        //! in \a str, doing a reverse search from \a index and searching at most \a num characters;
        //! returns \c UTFString::npos if nothing is found
        size_type find_last_of( const UTFString &str, size_type index = npos,
                                size_type num = npos ) const;
        //! returns the index of the first occurrence of \a ch in the current string, doing a reverse
        //! search from \a index; returns \c UTFString::npos if nothing is found
...
#    endif
        ...
        //! returns the index of the last character within the current string that does not match any
        //! character in \a str, doing a reverse search from \a index; returns \c UTFString::npos if
        //! nothing is found
        size_type find_last_not_of( const UTFString &str, size_type index = npos,
                                    size_type num = npos ) const;
        //! returns the index of the last occurrence of a character that does not match \a ch in the
        //! current string, doing a reverse search from \a index; returns \c UTFString::npos if nothing
        //! is found
...
#    endif
        ...
        //@{
        //! returns \c true if \a cp does not match the signature for the lead of follow code point of a
        //! surrogate pair in a UTF-16 sequence
        ...
        //! converts the given UTF-16 character buffer \a in_cp to a single UTF-32 Unicode character \a
        //! out_uc, returns the number of code points used to create the output character (2 for
        //! surrogate pairs, otherwise 1)
        ...
        //! writes the given UTF-32 \a uc_in to the buffer location \a out_cp using UTF-16 encoding,
        //! returns the number of code points used to encode the input (always 1 or 2)
        /*! This function, like its counterpart, will happily create invalid UTF-16 surrogate pairs.
        These invalid entries will be created for any value of \c in_uc that falls in the range U+D800 -
        U+DFFF. These are generally useful as sentinel values to represent various program specific
        conditions.
        ...
        //! converts the given UTF-8 character buffer to a single UTF-32 Unicode character, returns the
        //! number of bytes used to create the output character (maximum of 6)
        static size_t _utf8_to_utf32( const unsigned char in_cp[6], unicode_char &out_uc );
        //! writes the given UTF-32 \a uc_in to the buffer location \a out_cp using UTF-8 encoding,
...
#    endif
    ...
    // NB: many of these operations are evaluated in reverse because this is a reverse iterator wrapping
    // a forward iterator
    ...
    inline std::ostream &operator<<( std::ostream &os, const UTFString &s ) { return os << s.asUTF8(); }
